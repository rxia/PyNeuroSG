...
if False:
    ...
    [spcg1, spgc_t, spcg_f] = pna.ComputeSpectrogram(signal1, fs=fs, t_bin=t_bin, t_ini=t_ini)
    [spcg2, spgc_t, spcg_f] = pna.ComputeSpectrogram(signal2, fs=fs, t_bin=t_bin, t_ini=t_ini)
    [cohg, spgc_t, spcg_f]  = pna.ComputeCoherogram(signal1, signal2, fs=fs, t_bin=t_bin, t_ini=t_ini, tf_phase=True)
    ...
    plt.savefig('Showcase_LFP_spectrogram_coherence')
    plt.show()
...
if True:
    ...
    plt.axes(h_ax[1, 1])
    pnp.SpectrogramPlot(spcg_LFP, spgc_t, spcg_f, f_lim=[0, 120], tf_colorbar=True, tf_log=False)
