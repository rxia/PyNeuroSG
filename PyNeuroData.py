"""
Module to generate/manipulate/store the core data structure data_neuro,
which is trial aligned data for multiple channels [num_trials, num_timestamps, num_channels]
data generating part requires neo format for neuropysiologicla data; data stroage requires hdf5 interface
"""
...
def signal_align_to_evt(signal, evt_align_ts, window_offset, spike_bin_rate=1000):
    ...
    if type(signal) == neo.core.analogsignal.AnalogSignal:
        ...
        if 0:  # old, if return real time stamps, rather than binned counts
            ...
            signal_aligned = [signal[(signal > ts_window[i, 0]) * (signal < ts_window[i, 1])] - evt_align_ts[i] for i in
                              range(num_evts)]
        ...
        spk_binned = np.bincount(np.round((signal - t_start) * sampling_rate).astype('int'), minlength=num_bins)[
                     0:num_bins]
...
def signal_array_align_to_evt(segment, evt_align_ts, window_offset, type_filter='.*', name_filter='.*', chan_filter=[],
                              spike_bin_rate=1000):
    """
            ...
            for i in range(len(neo_data_object)):
                ...
                if re.match(name_filter, cur_name) is not None:
                    ...
                    if cur_chan in chan_filter:
                        ...
                        cur_signal_aligned = signal_align_to_evt(neo_data_object[i], evt_align_ts, window_offset,
                                                                 spike_bin_rate)
    ...
    if len(signal_aligned) == 0:
        ...
        print(
            'WARNING of function signal_array_align_to_evt: no signals in the segment match the selection filter for alignment')
        ...
        print(
            'WARNING of function signal_array_align_to_evt: signals are of different sampling rates, can not be combined:')
...
def blk_align_to_evt(blk, blk_evt_align_ts, window_offset, type_filter='.*', name_filter='.*', chan_filter=[],
                     spike_bin_rate=1000):
    """
    ...
    for i in range(len(blk.segments)):
        ...
        data_neuro = signal_array_align_to_evt(segment, evt_align_ts, window_offset, type_filter=type_filter,
                                               name_filter=name_filter, chan_filter=chan_filter,
                                               spike_bin_rate=spike_bin_rate)
...
def align_continuous(signal, t_start, sampling_rate, evt_align_ts, window_offset):
    ...
    try:
        ...
        warnings.warn(
            'function signal_align_to_evt() encounters {} events that are out of range'.format(n_rows_out_range))
...
""" ========== functions to adjust data_neuro ========== """
