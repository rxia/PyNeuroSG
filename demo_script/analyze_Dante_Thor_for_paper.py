...
path_to_fig = './temp_figs/final_Dante_Thor_paper'
...
def save_fig_all_format(fig_name='temp', path_to_fig=path_to_fig, format=('pkl', 'pdf', 'png'), h_fig=None):
    """ tool function to save figure """
    if h_fig is None:
        h_fig = plt.gcf()
    else:
    ...
    if 'pdf' in format:
        plt.savefig(os.path.join(path_to_fig, '{}.pdf'.format(fig_name)))
    if 'png' in format:
        plt.savefig(os.path.join(path_to_fig, '{}.png'.format(fig_name)))
    if 'pkl':
        with open(os.path.join(path_to_fig, '{}.pkl'.format(fig_name)), 'wb') as fid:
            pickle.dump(h_fig, fid)
...
for i, highlight in enumerate(plot_highlight):
    ...
    if i == 0:
        ...
        plt.legend(cdtn_name, fontsize='small')
    if i>=3:
        plt.xlabel('t (s)')
    # add cohen d test
    pnp.add_sub_axes(h_axes[i], loc='top', gap=0.05)
    if i == 0:
        plt.ylabel("Cohen's d")
    plt.title(highlight)
    if i>0:
        indx_traces = np.nonzero(plot_highlight[highlight]['trace'])[0]
        cohen_d_cur = pna.cal_CohenD(data_group_mean_norm[indx_traces[0]], data_group_mean_norm[indx_traces[-1]],
                                     axis=-1, type_test='paired')
        plt.fill_between(ts, 0, cohen_d_cur, facecolor=[0.7]*3, alpha=1.0, label='<0.2')
        plt.fill_between(ts, 0, np.where(np.abs(cohen_d_cur) > 0.2, cohen_d_cur, 0),
                         facecolor=[0.5]*3, alpha=1.0, label='0.2~0.5')
        plt.fill_between(ts, 0, np.where(np.abs(cohen_d_cur) > 0.5, cohen_d_cur, 0),
                         facecolor=[0.1]*3, alpha=1.0, label='>0.5')
        plt.ylim((-1.0, 1.0))
        if i == len(plot_highlight) - 1:
            plt.legend(bbox_to_anchor=(0.5, -0.2, 0.5, 0.5), fontsize='small')

plt.xlim([-0.050, 0.450])
save_fig_all_format(fig_name='PSTH_{}'.format(keep_mode))
...
def plot_fam_effect(signal_select):
    h_main, h_top, h_right = pnp.scatter_hist(fam_diff_early[signal_select], fam_diff_late[signal_select],
                     ...
                     kwargs_hist={'bins': np.arange(-1.4, 1.41, 0.10), 'color': 'k', 'alpha': 0.3})

    _, p_early = sp.stats.ttest_1samp(fam_diff_early[signal_select], 0)
    _, p_late  = sp.stats.ttest_1samp(fam_diff_late[signal_select], 0)

    plt.axes(h_top)
    plt.axvline(0, color='k', linestyle='--', alpha=0.5)
    plt.text(0.10, 0.8, 'p={:.2f}'.format(p_early), transform=plt.gca().transAxes)
    plt.text(0.5, 1.1, 'fam effect, early', transform=plt.gca().transAxes, fontsize='large', ha='center')

    plt.axes(h_right)
    plt.axhline(0, color='k', linestyle='--', alpha=0.5)
    plt.text(0.10, 0.9, 'p={:.2f}'.format(p_late), transform=plt.gca().transAxes)
    plt.text(1.1, 0.5, 'fam effect, late', transform=plt.gca().transAxes, fontsize='large', va='center', rotation=-90)
...
yn_keep_signal = signal_filter(keep_mode='IT')
plot_fam_effect(yn_keep_signal)
plt.text(0.05, 0.9, 'IT', transform=plt.gca().transAxes)

plt.xlim(-1.4, 1.4)
plt.ylim(-1.4, 1.4)

save_fig_all_format(fig_name='effect_fam_inhi_dist_{}'.format(cur_noise))


""" laminar distribution """

signal_all_spk['fam_effect'] = fam_diff_late
signal_all_spk['depth_by_2'] = np.array(signal_all_spk['depth'], dtype='int') // 2 * 2

plt.figure(figsize=(2, 4))
df_ana.DfPlot(signal_all_spk, 'fam_effect', x='depth_by_2', plot_type='box',
              limit=(keep_visual & (signal_all_spk['valid']==1)
                     & (signal_all_spk['depth']>=-8) & (signal_all_spk['depth']<=8)))
...
save_fig_all_format(fig_name='effect_fam_inhi_laminar_{}'.format(cur_noise))
...
if method_mean_latency == 'weighted_sum':
    ...
    data_baseline = np.mean(data_group_mean[:, (ts>-0.050) & (ts<0.050), :], axis=1, keepdims=True)
...
def plot_delay_effect(keep_signal_cur, title):
    plt.scatter(mean_latency[cond_compare[1], keep_signal_cur],
                mean_latency[cond_compare[0], keep_signal_cur], c=[0.2]*3, s=5, label='significant delay')
    plt.scatter(mean_latency[cond_compare[1], ~keep_noise_delay & keep_signal_cur],
                mean_latency[cond_compare[0], ~keep_noise_delay & keep_signal_cur], c=[0.5]*3, s=5, label='no delay')
    plt.plot([0.05, 0.25], [0.05, 0.25], color='gray', linestyle='--', alpha=0.5)
    plt.xlabel('lat, 70 noise')
    plt.ylabel('lat, 50 noise')
    plt.title('{}, {}/{}'.format(title, np.sum(keep_noise_delay & keep_signal_cur), sum(keep_signal_cur)))

keep_signal_cur = signal_filter(keep_mode='IT')
mean_latency_diff = mean_latency[cond_compare[1], keep_signal_cur] - mean_latency[cond_compare[0], keep_signal_cur]
mean_latency_diff_notnan = mean_latency_diff[np.isfinite(mean_latency_diff)]
_, p_val = sp.stats.ttest_1samp(mean_latency_diff_notnan, 0)

h_fig = plt.figure(figsize=[10, 4])
h_fig.patch.set_facecolor([0.9]*3)

plt.subplot('121')
plt.title('mean neural latency, {}'.format(fam_or_nov))
plot_delay_effect(keep_signal_cur, 'IT')
plt.xlim([0.05, 0.25])
plt.ylim([0.05, 0.25])
plt.xticks(np.arange(0.05, 0.251, 0.05))
plt.yticks(np.arange(0.05, 0.251, 0.05))
plt.gca().set_aspect('equal')

plt.subplot('222')
pnp.my_hist(mean_latency_diff_notnan, color='k', kwargs_hist={'bins': np.arange(-0.15, 0.15+0.01, 0.01)},
            yn_points=False, yn_median=False)
plt.text(0.10, 0.8, 'p={:.2f}'.format(p_val), transform=plt.gca().transAxes)

save_fig_all_format(fig_name='effect_delay_distr_{}'.format(fam_or_nov))


""" laminar distribution of noise delay effect """
...
df_ana.DfPlot(signal_all_spk, 'delay_effect', x='depth_by_2', plot_type='box', tf_count=False,
              limit=(keep_visual & (signal_all_spk['valid']==1)
                     & (signal_all_spk['depth']>=-8) & (signal_all_spk['depth']<=8)))
plt.axhline(0, color='gray', linestyle='--', alpha=0.5)
plt.axvline(4, color='gray', linestyle='--', alpha=0.5)
plt.ylim([-0.12, 0.12])
# plt.ylim([-0.040, 0.125])
# plt.ylim([-0.025, 0.100])

# for rotating the plot
plt.gca().invert_xaxis()
plt.xticks(rotation='vertical')
plt.yticks(rotation='vertical')

save_fig_all_format(fig_name='effect_delay_laminar_{}'.format(fam_or_nov))


""" SGr vs IGr delay effect comparison """
delay_effect_layer = (mean_latency[cond_compare[1], :] - mean_latency[cond_compare[0], :])
delay_effect_sGr = delay_effect_layer[signal_filter(keep_mode='IT_sGr')]
delay_effect_iGr = delay_effect_layer[signal_filter(keep_mode='IT_iGr')]
_, p_sGr = sp.stats.ttest_1samp(delay_effect_sGr, 0)
_, p_iGr = sp.stats.ttest_1samp(delay_effect_iGr, 0)
_, p_same = sp.stats.ttest_ind(delay_effect_sGr, delay_effect_iGr)


h_fig, h_axes = plt.subplots(2, 1, figsize=[4, 3], squeeze=False, sharex='all')
h_fig.patch.set_facecolor([0.9]*3)
plt.subplots_adjust(wspace=0, hspace=0)
h_axes = np.ravel(h_axes)

plt.axes(h_axes[0])
pnp.my_hist(delay_effect_sGr, color='k', kwargs_hist={'bins': np.arange(-0.12, 0.12+0.01, 0.01)},
            yn_points=False, yn_median=False)
plt.axvline(0, color='k', linestyle='--', linewidth=0.5)
plt.text(0.10, 0.8, 'sGr', transform=plt.gca().transAxes)
plt.text(0.10, 0.2,
         'p_sGr={:.3f} \np_iGr={:.3f} \np_same={:.3f}'.format(p_sGr, p_iGr, p_same),
         transform=plt.gca().transAxes)
plt.title('noise delay effect by cortical layers, {}'.format(fam_or_nov))

plt.axes(h_axes[1])
cur_neuron_idx = signal_filter(keep_mode='IT_iGr')
pnp.my_hist(delay_effect_iGr, color='k', kwargs_hist={'bins': np.arange(-0.12, 0.12+0.01, 0.01)},
            yn_points=False, yn_median=False)
plt.axvline(0, color='k', linestyle='--', linewidth=0.5)
plt.text(0.10, 0.8, 'iGr', transform=plt.gca().transAxes)
h_axes[1].invert_yaxis()

plt.xlabel('t (ms)')

plt.xlim(-0.12, 0.12)
save_fig_all_format(fig_name='noise delay effect by cortical layers, {}'.format(fam_or_nov))
...
save_fig_all_format(fig_name='rank_tuning_{}_{}'.format(keep_mode, t_mode))


""" test on slope """
shape_slope_all = np.array(tuning_curve.shape)[[0, 1, 3]]
tuning_slope = np.zeros(shape_slope_all)
tuning_slope_stderr = np.zeros(shape_slope_all)
for i_r in range(shape_slope_all[0]):
    for i_c in range(shape_slope_all[1]):
        for i_n in range(shape_slope_all[2]):
            slope, _, _, _, stderr = sp.stats.linregress(np.arange(tuning_curve.shape[2]), tuning_curve[i_r, i_c, :, i_n])
            tuning_slope[i_r, i_c, i_n] = slope
            tuning_slope_stderr[i_r, i_c, i_n] = slope


def custom_scatter_hist(data0, data1):
    lim = (-0.3, 0.1)

    h_main, h_top, h_right = pnp.scatter_hist(data0, data1,
                     kwargs_scatter={'s': 5, 'color': 'k', 'alpha': 0.9},
                     kwargs_hist={'bins': np.arange(lim[0], lim[1], 0.02), 'color': 'k', 'alpha': 0.3})

    _, p_same = sp.stats.ttest_rel(data0, data1)
    ...
    plt.axes(h_main)
    plt.plot([lim[0], lim[1]], [lim[0], lim[1]], color='k', linestyle='--', alpha=0.5)
    plt.text(0.20, 0.10, 'p={:.3f}'.format(p_same), transform=plt.gca().transAxes)
    plt.xlim(lim)
    plt.xlim(lim)



h_fig, h_axes = plt.subplots(1, 3, figsize=[12, 4])
plt.axes(h_axes[0])
custom_scatter_hist(tuning_slope[0, 0, yn_keep_signal], tuning_slope[1, 2, yn_keep_signal])
plt.xlabel('nov')
plt.ylabel('fam')
plt.title('fam vs nov')
plt.axes(h_axes[1])
custom_scatter_hist(tuning_slope[0, 0, yn_keep_signal], tuning_slope[0, 2, yn_keep_signal])
plt.xlabel('0, nov')
plt.ylabel('70, nov')
plt.title('nov, 70 vs 0')
plt.axes(h_axes[2])
custom_scatter_hist(tuning_slope[1, 0, yn_keep_signal], tuning_slope[1, 2, yn_keep_signal])
plt.xlabel('0, fam')
plt.ylabel('70, fam')
plt.title('fam, 70 vs 0')
plt.suptitle('tuning slope, {}'.format(t_mode))

save_fig_all_format(fig_name='rank_tuning_slope_{}'.format(t_mode))
...
i_ts_fano = [np.flatnonzero(ts > ts_cur)[0] for ts_cur in ts_fano]
...
sc_mean_sliding_window = sc_mean_all[:, :, :, i_ts_fano, :]
sc_std_sliding_window = sc_mean_all[:, :, :, i_ts_fano, :]
