""" script to explore the new data of Thor, both IT and V4 """
...
path_to_hdf5_data_Thor = '../support_data/data_neuro_Thor_Ruobing_all.hdf5'
path_to_hdf5_data = path_to_hdf5_data_Thor
path_to_fig = './temp_figs/V4_compare'
...
all_dates = list(tree_struct_hdf5_Thor.keys())
...
## plot by channel index for V4 channels
""" plot by channel index for V4 channels """
pash_by_V4_chan_shape = data_group_mean_norm.shape[:2] + (32,)
psth_by_V4_chan = np.zeros(pash_by_V4_chan_shape)
for i in range(32):
    yn_cur_chan = (signal_all_spk['area'] == 'V4') \
                  & (signal_all_spk['valid']==1) \
                  & (signal_all_spk['channel_index']==i+1) \
                  & (signal_all_spk['sort_code']>0)
    if sum(yn_cur_chan) > 0:
        psth_by_V4_chan[:, :, i] = np.mean(data_group_mean_norm[:, :, yn_cur_chan], axis=2)

##
yn_keep_signal = (signal_all_spk['area'] != 'V4') & (signal_all_spk['valid']==1)
h_fig, h_axes = pnp.create_array_layout_subplots(GM32_layout.layout_GM32_Thor, tf_text_ch=True, figsize=(10, 8))

for i in range(32):
    plt.axes(h_axes[i])
    for c in range(len(cdtn_name)):
        plt.plot(ts, psth_by_V4_chan[c][:, i],
                 color=colors[c], linestyle=linestyles[c])
    if i == 0:
        plt.legend(cdtn_name, fontsize='xx-small')
plt.xlim(-0.05, 0.4)
plt.xticks(np.arange(-0.1, 0.4, 0.1))
plt.suptitle('Thor, V4')
# plt.savefig(os.path.join(path_to_fig, 'Thor_V4_chan.png'))
# plt.savefig(os.path.join(path_to_fig, 'Thor_V4_chan.pdf'))
